...
class VectorStore:
    ...
    def delete_by_source(self, source: str):
        ...
        """
        with self.driver.session(database=self.database) as session:
            # Count first, then delete (DELETE removes the node so you can't
            # return count(c) after deletion — it will always be 0)
            ...
            print(f"🗑️  Deleted {total} chunks from source: {source}")
